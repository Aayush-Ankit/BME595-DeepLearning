...
function img2num.train ()
   ...
   for j = 1,max_iter do
      ...
      -- training
      invec_t = invec_t:view(in_size)
      local tr_out = net:forward(invec_t)
      tr_error[j] = loss:forward(tr_out, label_t)
      local gradLoss = loss:backward(tr_out, label_t)
      net:zeroGradParameters()
      net:backward(invec_t, gradLoss)
